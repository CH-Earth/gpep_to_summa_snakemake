#!/bin/bash
#SBATCH --job-name=process_snow_metrics
#SBATCH --output=process_snow_metrics_chena.out
#SBATCH --error=process_snow_metrics_chena.err
...
SCRIPT="/home/x-dcasson/GitRepos/gpep_to_summa_snakemake/workflow/scripts/process_water_year_snow_metrics_netcdf.py"
...
if [ ! -f "$SCRIPT" ]; then
    echo "Error: process_water_year_snow_metrics_netcdf.py not found at ${SCRIPT}" >&2
    exit 1
fi
