...
This repository contains a Snakemake workflow for converting GPEP meterological forcing data to SUMMA model input files.
...
This rule, named `add_gregorian_to_nc`, describes a process where a NetCDF file (`{id}.nc`) is taken from a directory (`config['gpep_forcing_dir']`), and a command is executed on it (`ncatted -a "calendar,time,o,c,"gregorian""`). The result is a new, temporary NetCDF file (`{id}_greg.nc`), stored in a different directory (`config['gpep_tmp_forcing_dir']`). The rule uses Python's pathlib.Path to create path objects and Snakemake's built-in `temp` function to specify that the output file is a temporary file that can be deleted as soon as no other rule needs it as input.
...
1. **Clone this git repository**
   ...
   Navigate to the local directory where the repo will be located. From your terminal, enter:
    ...
    `git clone https://github.com/DaveCasson/gpep_to_summa_snakemake.git`
...
2. **Set Up Virtual Environment (Optional)**  
   ...
   ***Option #1: Using venv directly***
     ...
     `python -m venv gpep_to_summa`
     `source gpep_to_summa/bin/activate`
  ...
  ```bash
   ...
   pyenv virtualenv 3.9.16 gpep_to_summa_snakemake

   pyenv activate gpep_to_summa_snakemake
   ```
...
3. **Install Dependencies**  
  ...
  Navigate to the gpep_to_summa_snakemake repo directory, with the environment activated.
...
4. **Install gpep_to_summa_snakemake as kernel**

    ```bash
    ipython kernel install --name "gpep_to_summa_snakemake" --user
    ```
...
7. **Run simple snakemake workflow**
    ...
    Run the first snakemake file from the Notebook (../rules/gpep_file_prep.smk) from the notebook. This will be running the first few Jupyter notebook cells.
    ...
    ***Start Jupyter Notebook***
      ...
      jupyter notebook
      ```
     Once this installation is complete, further instructions for running the snakemake workflows are located in the [workflow](https://github.com/DaveCasson/gpep_to_summa_snakemake/tree/main/workflow) directory of this repo.


## Acknowledgements

gpep data, and an original processing workflow was created by Andy Wood.
