...
config['easymore_input_dir'] = config['grid_input_dir']
config['easymore_output_dir'] = config['grid_output_dir']
config['easymore_temp_dir'] = config['grid_temp_dir']
remapping_file_name = config['case_name'] + "_remapping.nc"
config['remap_file'] = Path(config['grid_temp_dir'],remapping_file_name)
config['easymore_input_var'] = config['grid_input_var']
final_nc_name = config['case_name'] + "_remapped.nc"

file_path_list = sorted([f for f in os.listdir(config['grid_input_dir']) if f.endswith('.nc')])
print(f"File path list: {file_path_list}")

# Now you can grab the "first" entry
first_file = (Path(config['grid_input_dir']) / file_path_list[0]).with_suffix(".nc")
print(f"First file: {first_file}")

rule remap_grid_to_shp:
    input:
       Path(config['grid_output_dir'],"final", final_nc_name)
...
rule merge_grid_files_with_cdo:
    ...
    output:
        output_file = Path(config['grid_output_dir'],"final", final_nc_name)
    resources:
        mem_mb = 10000,
        runtime = 10
    shell:
