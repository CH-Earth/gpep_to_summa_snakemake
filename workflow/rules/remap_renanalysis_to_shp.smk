# Snakefile (ERA5-only, simplest: pass Easymore a list of var names)
...
# ERA5: pass Easymore a simple ordered list of variable names (no dict, no keys)
# Expected Easymore order: [prcp, tair, psfc, qair, wnd, lwdown, swdown]
config["easymore_input_var"] = [
    "pptrate",   # prcp
    "airtemp",   # tair
    "airpres",   # psfc
    "spechum",   # qair
...
rule process_netcdf:
    ...
    params:
        dt = 3600  # hourly in your ERA5 example
    resources:
        mem_mb = 10000,
        runtime = 20
    shell:
        ...
        # ERA5 already uses SUMMA names and SI values; just normalize unit labels.

        # 1) Stamp units (labels only; values unchanged)
        ...
        # (Optional) Standard names — uncomment if you want to enforce these too:
        # ncatted -O -a standard_name,airtemp,o,c,'air_temperature' "$tmp"
        # ncatted -O -a standard_name,airpres,o,c,'surface_air_pressure' "$tmp"
        # ncatted -O -a standard_name,spechum,o,c,'specific_humidity' "$tmp"
        # ncatted -O -a standard_name,windspd,o,c,'wind_speed' "$tmp"
        # ncatted -O -a standard_name,LWRadAtm,o,c,'surface_downwelling_longwave_flux' "$tmp"
        # ncatted -O -a standard_name,SWRadAtm,o,c,'surface_downwelling_shortwave_flux' "$tmp"
        # ncatted -O -a standard_name,pptrate,o,c,'precipitation_flux' "$tmp"

        # 2) Time handling (kept to mirror your CASR pipeline)
        ...
        # 3) HRU dim/var normalization
        ncrename -O -d .ID,hru "$tmp" || true
        ncrename -O -v .ID,hruId "$tmp" || true

        # 4) Add hru coordinate variable and metadata
